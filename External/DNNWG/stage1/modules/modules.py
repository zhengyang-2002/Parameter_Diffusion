...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 2), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
                 resolution=32, z_channels, double_z=True, use_linear_attn=False, attn_type="vanilla", in_dim=2864,
                 fdim=8192, my_channels=24, **ignore_kwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 2), num_res_blocks,
                 attn_resolutions, dropout=0.0, resamp_with_conv=True, in_channels,
                 resolution=32, z_channels=16, give_pre_end=False, tanh_out=False, use_linear_attn=False,
                 attn_type="vanilla", in_dim=2864, fdim=8192, my_channels=24, **ignorekwargs):
        super().__init__()
        if use_linear_attn: attn_type = "linear"
        self.ch = ch
        self.temb_ch = 0
...
class NNPredictor(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1, 2, 4, 2), num_res_blocks,
        ...
        self.fc_in = nn.Linear(self.in_dim,fdim)
        # self.fc_in = nn.Linear(self.in_dim, resolution * resolution * in_channels)
        factor = 2 * z_channels if double_z else z_channels
        out_dim = (factor * resolution ** 2 )/ (len(ch_mult - 1))
    ...
    def forward(self, x):
        ...
        h = self.fc_out(h)
        h= torch.sigmoid(h)
        # self.fc_out = nn.Linear(selout_dim, 1)
        return h
...
class MyEncoder(nn.Module):
    def __init__(self, in_dim, z_channels, input_size=64, in_channels=24, double_z=True):
        ...
        self.conv3 = nn.Conv2d(32, self.z_channels, kernel_size=3, stride=2, padding=1, bias=False)
...
class MyDecoder(nn.Module):
    def __init__(self, in_dim, z_channels, input_size=64, in_channels=24, double_z=True):
        ...
        # self.conv = nn.Conv2d(32, self.in_channels, kernel_size=3, stride=1, padding=1, bias=False)
